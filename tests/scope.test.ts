...
describe("argv", () => {
  it("requires scope and a change", () => {
    ...
    if (withSearch.ok) assert.equal(withSearch.search, true);
  });
});
...
describe("osi scope against fixture", () => {
  ...
  it("fails for missing and archived ids", () => {
    assert.throws(() => runScope({ cwd: fixture, change: "does-not-exist", includeLow: false }), LocateError);
    assert.throws(() => runScope({ cwd: fixture, change: "old-thing", includeLow: false }), LocateError);
  });
  ...
  it("omits low by default and includes it with --include-low", () => {
    ...
    assert.equal(def.candidates.some((c) => c.confidence === "low"), false);
    assert.ok(all.candidates.some((c) => c.path === "src/extra/notes.txt" && c.confidence === "low"));
  });
  ...
  it("splits tests and sets related_to", () => {
    const doc = runScope({ cwd: fixture, change: "add-renewal-status", includeLow: false });
    assert.equal(doc.candidates.some((c) => c.path.includes(".test.")), false);
  ...
  it("does not emit openspec markdown as candidates", () => {
    ...
    assert.equal(doc.candidates.some((c) => c.path.startsWith("openspec/")), false);
    assert.equal(doc.tests.some((t) => t.path.startsWith("openspec/")), false);
  });
