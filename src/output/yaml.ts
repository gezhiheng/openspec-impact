...
export function toYaml(doc: ScopeDocument): string {
  const lines: string[] = ["version: 1", "change:", `  name: ${y(doc.change.name)}`, `  path: ${y(doc.change.path)}`];

  if (doc.concepts.length === 0) lines.push("concepts: []");
  else {
        lines.push("concepts:");
  ...
  if (doc.candidates.length === 0) lines.push("candidates: []");
  else {
  ...
  if (doc.tests.length === 0) lines.push("tests: []");
  else {
    lines.push("tests:");
    for (const t of doc.tests) {
      lines.push(`  - path: ${y(t.path)}`);
      if (t.related_to) lines.push(`    related_to: ${y(t.related_to)}`);
    }
  }
