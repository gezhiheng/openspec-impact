...
export function parseArgv(argv: string[]): ParsedArgs {
  ...
  for (const arg of rest) {
    if (arg === "--include-low") includeLow = true;
    else if (arg === "--search") search = true;
    else if (arg.startsWith("-")) return { ok: false, message: `Unknown flag: ${arg}\n${USAGE}` };
    else positional.push(arg);
  }
  const command = positional[0];
  if (!command) return { ok: false, message: USAGE };
  if (command !== "scope") return { ok: false, message: `Unknown command: ${command}\n${USAGE}` };
  const change = positional[1];
  if (!change || positional.length > 2) return { ok: false, message: USAGE };
...
function isDirectRun(): boolean {
  ...
  try {
    ...
    return entry.replaceAll("\\", "/").endsWith("/cli.js") || entry.replaceAll("\\", "/").endsWith("/cli.ts");
  }
}
