...
- backtick, inline-code, and bold tokens
- path-like tokens
- kebab-case pieces of the change directory name and capability directory names
- 2–3 token phrases from headings, `### Requirement` lines, and the proposal sections What Changes and Impact

It MUST NOT n-gram scenario / Why body prose. It MUST NOT translate Chinese into English. Tokens that appear in the never-search list MUST NOT become search terms by themselves. Tokens in the path-only list MAY be used only for path matching, never as content or symbol queries. Multi-word phrases that contain a path-only word (for example `tenant list`) MUST still be expanded and fully searched.
...
The CLI MUST NOT emit a `type` field on concepts.

Never-search (standalone): `should` `must` `shall` `may` `system` `user` `when` `then` `given` `and` `the` `a` `an` `to` `of` `in` `on` `for` `with` `by` `from` `this` `that` `support` `display` `add` `added` `change` `changes` `requirement` `scenario` `purpose` `why` `what`

Path-only: `filter` `export` `search` `create` `update` `renew` `list` `detail` `page` `status` `form` `view` `modal` `dialog` `table` `button`
